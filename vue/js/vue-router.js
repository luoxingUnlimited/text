    ...
    function parse(str, options) {
        ...
        while ((res = PATH_REGEXP.exec(str)) != null) {
            ...
            tokens.push({
                ...
                partial: partial,
                asterisk: !!asterisk,
                pattern: pattern ? escapeGroup(pattern) : (asterisk ? '.*' : '[^' + escapeString(
                    delimiter) + ']+?')
            });
        }
    ...
    function tokensToFunction(tokens, options) {
        ...
        return function (obj, opts) {
            ...
            for (var i = 0; i < tokens.length; i++) {
                ...
                if (isarray(value)) {
                    if (!token.repeat) {
                        throw new TypeError('Expected "' + token.name + '" to not repeat, but received `' +
            ...
            flatMapComponents(matched, function (def, _, match, key) {
                ...
                if (typeof def === 'function' && def.cid === undefined) {
                    ...
                    var reject = once(function (reason) {
                        var msg = "Failed to resolve async component " + key + ": " +
                            reason;
                        warn(false, msg);
                        if (!error) {
